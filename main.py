...
from model import *
from train import train_dssm, train_disc
...
# model
parser.add_argument('--infer_model', type=str, default='STLR', choices=['STLR', 'STR', 'STL', 'MFLR'])
...
def initialize():
    ...
    train_loader_kwargs = {
        'data_path':args.dataset_path + args.evaluation + '/train_data.npy',
        'label_path':args.dataset_path + args.evaluation + '/train_label.pkl',
    ...
    test_loader_kwargs ={
        ...
        'noise_addition':args.noise_addition,
        'noise_scale':args.noise_scale}
    return train_loader_kwargs, test_loader_kwargs

def main():
    initialize()
    device = torch.device("cuda")

    train_loader_kwargs, test_loader_kwargs = initialize()
    model = Net(args, device).to(device)

    train_dssm(args, model, device, train_loader_kwargs, test_loader_kwargs)
    train_disc(args, model, device, train_loader_kwargs, test_loader_kwargs)
